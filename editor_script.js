...
window.correctionRequest = correctionRequest;
...
// === 안정형 탭 전환 (재귀 제거 + 스코프 고정) ===
(function () {
  // 내부 원본 (스코프: #utility)
  function _switchTab(tabName, event) {
    // 0) #utility 컨테이너 스코프 확정
    const box = document.getElementById('utility');
    if (!box) return;
    ...
    const btn = (event && event.currentTarget) || box.querySelector(`.tab-btn[data-tab="${tabName}"]`);
    if (btn) btn.classList.add('active');

    const panel = box.querySelector('#' + tabName + '-tab');
    if (panel) panel.classList.add('active');
  }

  // 전역 API: 재귀 없이 내부 함수만 래핑
  window.switchTab = function (tabName, event) {
    try {
      _switchTab(tabName, event || window.event);
    } catch (e) {
...
function addChatMessage(msg) {
  ...
  if (!msg.isMine) {
    const chatTab = document.querySelector('#utility .tab-btn[data-tab="chat"]');
...
function updateChatBadge() {
  ...
  // 2) (선택) 버튼에 강조 클래스를 주고 싶다면 data-tab으로 특정
  const chatTabBtn = document.querySelector('#utility .tab-btn[data-tab="chat"]');
  if (chatTabBtn) {
    if (typeof unreadMessages !== 'undefined' && unreadMessages > 0) {
